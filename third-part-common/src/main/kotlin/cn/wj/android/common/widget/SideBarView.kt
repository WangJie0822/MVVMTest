...
class SideBarView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    /** 文本、字母导航集合 */
    private var letters = arrayOf("新", "手", "#", "A", "B", "C", "D", "E", "F", "G", "H", "I",
    ...
    init {
        ...
        bgColorPressed = typedArray.getColor(R.styleable.SideBarView_sidebar_press_background, Color.parseColor("#FFF3F3F3"))
        textSize = typedArray.getDimension(R.styleable.SideBarView_sidebar_text_size, 18f).toInt()
        textColorNormal = typedArray.getColor(R.styleable.SideBarView_sidebar_text_color_nor, Color.parseColor("#FF7F7F7F"))
        textColorPressed = typedArray.getColor(R.styleable.SideBarView_sidebar_text_color_press, Color.parseColor("#FF7F7F7F"))
        typedArray.recycle()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in letters.indices) {
            canvas.drawText(letters[i], mWidth / 2 - paintNormal.measureText(letters[i]) / 2, (perHeight * i + perHeight).toFloat(), paintNormal)
            if (selectPos == i) {
                canvas.drawText(letters[i], mWidth / 2 - paintNormal.measureText(letters[i]) / 2, (perHeight * i + perHeight).toFloat(), paintSelect)
            }
        }
    }
